...
import {
  ...
  ChevronDown, ChevronUp, Briefcase, CreditCard, Calendar,
} from 'lucide-react';
import {
...
function SalarySlipPreview({
  ...
  return (
    <div
      ...
      className="bg-white text-gray-900 w-full rounded-2xl shadow-2xl overflow-hidden font-sans"
      style={{ fontFamily: 'Inter, Arial, sans-serif', fontSize: '13px', minWidth: 480 }}
    >
      {/* Header */}
      <div className="bg-gradient-to-r from-violet-700 to-indigo-700 text-white px-8 py-6">
        <div className="flex justify-between items-start">
          <div>
            <h2 className="text-xl font-bold tracking-tight">{company.companyName || 'Company Name'}</h2>
            <p className="text-violet-200 text-xs mt-1 max-w-xs">{company.companyAddress || 'Company Address'}</p>
            {company.companyEmail && <p className="text-violet-200 text-xs">{company.companyEmail}</p>}
            {company.companyPhone && <p className="text-violet-200 text-xs">{company.companyPhone}</p>}
            {company.cin && <p className="text-violet-200 text-xs">CIN: {company.cin}</p>}
          </div>
          <div className="text-right">
            <div className="text-violet-200 text-xs font-semibold uppercase tracking-widest">Payslip</div>
      ...
      <div className="bg-violet-50 border-b border-violet-100 px-8 py-4">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            <div key={l}>
              <div className="text-violet-500 text-[10px] font-bold uppercase tracking-wider">{l}</div>
              <div className="text-gray-800 font-semibold text-xs mt-0.5">{v}</div>
            </div>
          ) : null)}
        </div>
        <div className="mt-3 flex gap-6 text-xs">
          <span><span className="text-violet-500 font-bold">Working Days:</span> <span className="font-semibold">{employee.workingDays}</span></span>
          <span><span className="text-violet-500 font-bold">Paid Days:</span> <span className="font-semibold">{employee.paidDays}</span></span>
        </div>
      </div>
        ...
        <table className="w-full text-xs border-collapse">
          ...
          <tfoot>
            <tr className="bg-gray-50 font-bold border-t-2 border-gray-300">
              ...
              <td className="py-3 px-3 text-right text-violet-700">₹{fmt(result.grossEarnings)}</td>
      ...
      {/* Net Pay Banner */}
      <div className="mx-8 mb-6 mt-4 rounded-xl bg-gradient-to-r from-violet-600 to-indigo-600 px-6 py-4 flex justify-between items-center text-white">
        <div>
          <div className="text-violet-200 text-xs font-semibold uppercase tracking-widest">Net Pay</div>
          <div className="text-2xl font-black mt-0.5">₹{fmt(employee.workingDays > 0 ? effectiveNet : result.netPay)}</div>
          <div className="text-violet-200 text-xs mt-1 italic">{result.netPayWords}</div>
        </div>
        {employee.ifscCode && (
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-8 items-start">
        ...
        <div className="space-y-4">
          {/* Company */}
          <Section title="Company Details" icon={<Building2 size={16} className="text-violet-500" />}>
          ...
          {/* Employee */}
          <Section title="Employee Details" icon={<User size={16} className="text-violet-500" />}>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
